...
function AddBotEsp(Path)
    ...
    renderstepped = game:GetService("RunService").RenderStepped:Connect(function()
        ...
        if Path and (game:GetService("Workspace").AiZones:FindFirstChild(Path.Name, true)) and Path:FindFirstChildOfClass("Humanoid") and Path:FindFirstChildOfClass("Humanoid").Health > 0 then
            ...
            if esp.customsettings.enabled and esp.customsettings.ai.enabled and Path:FindFirstChildOfClass("MeshPart") and (esp.customsettings.maxdist == 0 or (Path.MeshPart.Position - plr.Character.HumanoidRootPart.Position).Magnitude < esp.customsettings.maxdist) then
    ...
    renderstepped = game:GetService("RunService").RenderStepped:Connect(function()
       --[[ if esp.customsettings.corpsechams.enabled then
        ...
        if Corpse and workspace.DroppedItems:FindFirstChild(Corpse.Name) and Corpse:FindFirstChildOfClass("Humanoid") then
            ...
            if esp.customsettings.enabled and esp.customsettings.corpse.enabled and Corpse:FindFirstChildOfClass("MeshPart") and (esp.customsettings.maxdist == 0 or (Path.MeshPart.Position - plr.Character.HumanoidRootPart.Position).Magnitude < esp.customsettings.maxdist) then
                ...
                if drop_onscreen then
                    ...
                    if esp.customsettings.corpsedistance.enabled then
                        if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                            CorpseEsp2.Text = math.round((Corpse:FindFirstChildOfClass("MeshPart").Position-game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude / 3) .. "m"
...
function AddExtractEsp(Extract)
    ...
    renderstepped = game:GetService("RunService").RenderStepped:Connect(function()
        if Extract then
            if esp.customsettings.enabled and esp.customsettings.extract.enabled and (esp.customsettings.maxdist == 0 or (Path.MeshPart.Position - plr.Character.HumanoidRootPart.Position).Magnitude < esp.customsettings.maxdist) then
